  ...
  return (
    ...
    >
      <div style={{ background: '#FFFFFF', borderRadius: '16px', width: '100%', maxWidth: '480px', boxShadow: '0 20px 60px rgba(0,0,0,0.2)', overflow: 'hidden' }}>
        {/* Header */}
        <div style={{ background: 'var(--color-secondary)', padding: '1.5rem 1.75rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <div style={{ color: 'var(--color-primary)', fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.35rem' }}>{requestType.replace(/-/g, ' ')}</div>
        ...
        <div style={{ padding: '1.75rem' }}>
          {status === 'success' ? (
            <div style={{ textAlign: 'center', padding: '1.5rem 0' }}>
              <div style={{ width: '56px', height: '56px', background: 'var(--color-secondary)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1rem' }}>
                <svg width="24" height="24" fill="none" stroke="var(--color-primary)" strokeWidth="2.5" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>
              </div>
              <h4 style={{ fontWeight: 800, color: 'var(--color-primary)', marginBottom: '0.5rem' }}>Request Received!</h4>
              <p style={{ color: '#6B7280', fontSize: '0.875rem', marginBottom: '1.25rem' }}>
                Our team will contact you within 24 hours to follow up.
              </p>
              <button onClick={onClose} style={{ background: 'var(--color-primary)', color: '#FFFFFF', border: 'none', borderRadius: '8px', padding: '0.65rem 1.5rem', fontWeight: 700, cursor: 'pointer', fontSize: '0.875rem' }}>
                Close
              </button>
            </div>
          ) : (
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <button
                ...
                style={{ background: 'var(--color-primary)', color: '#FFFFFF', border: 'none', borderRadius: '8px', padding: '0.85rem', fontWeight: 700, cursor: status === 'loading' ? 'not-allowed' : 'pointer', fontSize: '0.95rem', opacity: status === 'loading' ? 0.7 : 1 }}
              >
